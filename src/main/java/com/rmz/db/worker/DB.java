...
import com.rmz.db.worker.utils.Props;
...
public class DB {
    ...
    public DB(String dbName) {
        ...
        DBConfig dbc = new DBConfig(dbName, Props.get("db." + dbName + ".server"),
                Props.get("db." + dbName + ".username"), Props.get("db." + dbName + ".password"));
        // Open DB connection
        openConnection(dbc);
    }
    ...
    public DB(String dbName, String dbServer, String dbUser, String dbPassword) {
        LOGGER.info("Connection to DB -> " + dbName);
        DBConfig dbc = new DBConfig(dbName, dbServer, dbUser, dbPassword);
        // Open DB connection
        openConnection(dbc);
    }
     ...
     *
     * @param dbc - DB configuration
     */
    private void openConnection(DBConfig dbc) {
        try {
    ...
    public DB query(String sql) throws SQLException {
        ...
        boolean isDML = sql.toUpperCase().contains("UPDATE") ||
                sql.toUpperCase().contains("DELETE") ||
                sql.toUpperCase().contains("INSERT") ||
                sql.toUpperCase().contains("DECLARE") ||
